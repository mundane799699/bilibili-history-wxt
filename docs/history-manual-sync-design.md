...
> 本文同时作为实现说明与验收依据。
>
> 2026-08-04 更新：同步弹窗和后台协议已扩展为 `incremental / smart / full` 三种模式。智能同步为默认推荐项：完整获取服务器当前可用历史，再精确对齐严格晚于服务器最早记录的本地数据，同时保留更早的本地历史。本文后续仍出现的 `isFullSync` 两模式描述仅保留为最初设计记录。

## 1. 结论

在 `pages/History.tsx` 顶部操作区新增“同步历史记录”按钮。点击后打开模态框，用户可选择增量、智能或全量同步；同步期间在弹窗内展示进行中状态，完成后展示后台返回结果，并自动刷新历史列表和总记录数。
