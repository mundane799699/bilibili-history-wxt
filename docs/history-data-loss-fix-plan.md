...
它只用于定时增量同步。智能同步不得使用该条件，必须遍历到服务器返回空页，取得完整的服务器保留窗口。

### 5.3 智能同步按服务器保留窗口对齐

智能同步执行：

1. 从最新页开始，使用完整游标翻页到服务器返回空列表。
2. 以快照中的最小 `view_at` 作为服务器当前最早边界。
3. 新增或更新快照中的全部记录。
4. 删除本地中严格晚于该边界、但不在服务器快照中的记录。
5. 本地中早于或等于该边界的记录保持不变。
6. 只有完整快照成功获取后，才在单个 IndexedDB 事务中执行写入和删除。

服务器返回空快照时没有可用边界，因此不得删除任何本地记录。

全量同步仍只能新增或更新，不得在开始前清空本地历史；智能同步则是用户明确触发的服务器窗口精确对齐操作。
...
- HTTP、JSON、IndexedDB 任一环节失败都不能更新同步成功时间，也不能执行不完整快照的删除。
...
- 页尾为番剧或直播时仍能获取下一页。
- 智能同步即使首页首尾记录存在，仍会遍历到服务器返回空页。
- 游标停滞、页面重复、HTTP 错误和事务失败均不执行快照对齐或更新成功时间。
- 全量和增量同步前后记录数不会下降；智能同步只删除服务器保留窗口内已消失的记录。
...
- [ ] 第 4 节：堵住不可恢复的删除路径
- [x] 第 5 节：修复历史同步不完整
  - [x] 传递 `business / max / view_at` 完整游标
  - [x] 智能同步完整抓取服务器保留窗口，不依赖首页首尾 ID
  - [x] 在单个事务内新增、更新并删除服务器窗口内已消失的记录
  - [x] 保留早于或等于服务器最早时间的本地历史
  - [x] 游标停滞、重复页面、异常响应或写入失败时不执行快照对齐
